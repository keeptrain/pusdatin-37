	...
	@can('view pr request', $this->publicRelations)
		...
		<flux:table.base :perPage="$perPage" :paginate="$this->publicRelations"
			...
			<x-slot name="header">
				...
				<flux:table.column>Tanggal Selesai</flux:table.column>
			...
			<x-slot name="body">
				@foreach ($this->publicRelations as $item)
					<tr @click="$wire.show({{ $item->id }})" class="{{ in_array($item->id, $selectedPrRequest) ? 'relative bg-zinc-50 dark:bg-zinc-900 ' : 'dark:bg-zinc-800' }}
