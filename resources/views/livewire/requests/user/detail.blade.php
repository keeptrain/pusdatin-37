<section>
    ...
    @if ($type === 'information-system')
        @if (!$this->needUploadAdditionalFile)
            <flux:callout icon="exclamation-triangle" color="yellow" class="mb-3">
                <flux:callout.text>
                    ...
                    <flux:callout.link x-on:click="$dispatch('modal-show', { name: 'upload-modal' });" class="cursor-pointer">
        ...
        <x-user.information-system.card-basic-info :id="$content->id" :created-at="$content->createdAtDMY()"
            ...
            :person="$content->user->name" :activerevision="$content->active_revision" :uploadedFile="$this->uploadedFile"
            :meeting="$content->meeting" :referenceNumber="$content->reference_number" />
