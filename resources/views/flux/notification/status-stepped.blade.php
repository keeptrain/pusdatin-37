        ...
        <div class="relative flex items-center justify-between w-full">
            ...
            @foreach($statuses as $index => $stepStatus)
                <div class="relative flex flex-col items-center">
                    {{-- Dot --}}
                    <div class="relative z-10 grid w-4 h-4 rounded-full place-items-center font-bold transition-all duration-300
                        @if ($index < $currentIndex || $index == $currentIndex)
                            {{ $index == 3 && $isRejected ? 'bg-red-500 dark:bg-red-600' : 'bg-zinc-600 dark:bg-zinc-500 text-white' }}
                        @else
                    ...
                    {{-- Label --}}
                    @if($showLabels ?? true)
                        <span class="absolute mt-6 text-xs font-medium text-center whitespace-nowrap
            ...
            <div class="relative flex items-center mb-10 last:mb-0">
                ...
                <div class="relative z-20 w-4 h-4 rounded-full flex items-center justify-center font-bold
                    @if ($index <= $currentIndex)
                        {{ $index == 3 && $isRejected ? 'bg-red-500 dark:bg-red-600 text-white' : 'bg-blue-600 dark:bg-blue-500 text-white' }}
                    @else
                        bg-gray-300 dark:bg-gray-500 text-gray-900 dark:text-gray-100
                    @endif">
                </div>
