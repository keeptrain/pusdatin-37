...
<div class="space-y-2">
    ...
    <input type="file" wire:model="imagesUpload" accept="image/*" multiple class="block text-sm text-gray-500
    ...
    @if ($form->attachments)
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            @foreach ($form->attachments as $index => $image)
                <div class="relative group rounded-md overflow-hidden border">
                    ...
                    <button type="button" x-on:click="$wire.removeTemporaryImage({{ $index }})"
